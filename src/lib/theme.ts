 ...
 * Values traced 1:1 from the Flutter app and the design handoff's kuber-theme.js.
 *
 * This is the single source of truth for colors. `scripts/gen-tokens.mjs` reads
 * it to emit `src/styles/tokens.css` with every derived token pre-computed, so
 * the CSS never hand-types a color.
 */
...
export interface ThemeMeta {
  id: string;
  name: string;
  /** Short colour word shown next to the name. */
  label: string;
}
...
export const PALETTES: Record<string, Record<Mode, Palette>> = {
  signature: {
...
export function deepShade(hex: string): string {
  ...
  const q = L < 0.5 ? L * (1 + s) : L + s - L * s,
    p = 2 * L - q;
  const to = (x: number): string => Math.round(hue2rgb(p, q, x) * 255).toString(16).padStart(2, '0');
  return `#${to(hue + 1 / 3)}${to(hue)}${to(hue - 1 / 3)}`;
}

/** All CSS custom properties for one palette, including derived tokens. */
export function paletteVars(p: Palette, mode: Mode): Record<string, string> {
  const subtle = mode === 'light' ? 0.1 : 0.14;
  const ring = mode === 'light' ? 0.24 : 0.3;
  return {
    '--k-bg': p.bg,
    '--k-card': p.card,
    '--k-muted': p.muted,
    '--k-border': p.border,
    '--k-border-muted': p.borderMuted,
    '--k-text': p.text,
    '--k-text-sec': p.textSec,
    '--k-accent': p.accent,
    '--k-accent-subtle': hexToRgba(p.accent, subtle),
    '--k-accent-ring': hexToRgba(p.accent, ring),
    '--k-on-accent': p.onAccent,
    '--k-accent-text': p.accentText,
    '--k-income': p.income,
    '--k-income-subtle': hexToRgba(p.income, subtle),
    '--k-expense': p.expense,
    '--k-expense-subtle': hexToRgba(p.expense, subtle),
    '--k-warning': p.warning,
    '--k-tile': deepShade(p.accent),
    'color-scheme': mode === 'light' ? 'light' : 'dark',
  };
}

/** Accent swatch for a family in a given mode (used by the theme switcher UI). */
export function swatch(id: string, mode: Mode): string {
  return (PALETTES[id] ?? PALETTES.signature)[mode].accent;
}
